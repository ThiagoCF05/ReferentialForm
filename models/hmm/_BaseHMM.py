...
class _BaseHMM(object):
    ...
    def __init__(self, n, m, precision=numpy.double, verbose=False,scaled=True):
        self.n = n
        self.m = m
    ...
    def forwardbackward(self, observations, cache=False):
        ...
        if self.scaled:
            alpha, c = self._calcsalpha(observations)
            return -sum(numpy.log(c))
        else:
    ...
    def _calcsalpha(self, observations):
        '''
        Calculates scaled 'alpha' the forward variable.
        ...
        '''
        alpha = numpy.zeros((len(observations), self.n), dtype=self.precision)
        c = numpy.zeros((len(observations)), dtype=self.precision)
    ...
    def _calcsbeta(self, observations, c):
        '''
        Calculates 'beta' the backward variable.
        ...
        # init stage
        for s in xrange(self.n):
            beta[len(observations) - 1][s] = c[len(observations)-1]
    ...
    def _scale(self, observations):
        '''
        Calculates scaled 'alpha' and 'beta'
        '''
        alpha, c = self._calcsalpha(observations)
        beta = self._calcsbeta(observations, c)
    ...
    def _calcxi(self, observations, alpha=None, beta=None):
        ...
        '''
        if self.scaled:
            _alpha, _beta = self._scale(observations)
            if alpha is None:
                alpha = _alpha
            if beta is None:
                beta = _beta
    ...
    def train(self, observations, iterations=0, epsilon=0.0001, thres=-0.001):
        '''
        ...
        for i in xrange(iterations):
            ...
            if (self.verbose):
                print "iter: ", i, ", L(model|O) =", prob_old, ", L(model_new|O) =", prob_new, ", converging =", (
                prob_new - prob_old > thres )
    ...
    def _calcstats(self, observations):
        ...
        if self.scaled:
            stats['alpha'], stats['beta'] = self._scale(observations)
        else:
